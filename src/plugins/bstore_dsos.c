...
#include "config.h" /* form automake */
...
static int bstore_lock = 0;

static time_t clamp_time_to_bin(time_t time_, uint32_t bin_width)
{
	return (time_ / bin_width) * bin_width;
}

typedef struct bsos_iter_s *bsos_iter_t;
typedef struct bstore_dsos_s {
	struct bstore_s base;
	dsos_session_t sess;	/* The D/SOS session */
	dsos_container_t dict_sos;
	dsos_container_t ptn_sos;
	dsos_container_t ptn_tkn_sos;
	dsos_container_t msg_sos;
	dsos_container_t hist_sos;
	dsos_container_t attr_sos;

	dsos_schema_t token_value_schema;
	dsos_iter_t tkn_id_iter;
	dsos_iter_t tkn_text_iter;
	dsos_schema_t message_schema;
	dsos_iter_t msg_iter;
	dsos_schema_t pattern_schema;
	dsos_iter_t ptn_iter;
	dsos_schema_t pattern_token_schema;
	dsos_iter_t ptn_pos_tkn_iter;
	dsos_schema_t token_hist_schema;
	dsos_schema_t pattern_hist_schema;
	dsos_schema_t component_hist_schema;
	dsos_schema_t attribute_schema;
	dsos_schema_t pattern_attribute_schema;
...
#define HIST_IDX_TYPE "H2BXT"
#define HIST_IDX_ARGS "ORDER=5 SIZE=11"
#define IDX_TYPE "BXTREE"
#define IDX_ARGS "ORDER=5"
typedef struct ptn_tkn_value_s {
	uint64_t ptn_id;
	uint64_t pos;
	uint64_t tkn_id;
	uint64_t tkn_count;
...
struct sos_schema_template pattern_token_schema = {
	...
	.attrs = {
		...
		{
			.name = "pos", /* token index in pattern */
			.type = SOS_TYPE_UINT64,
		},
		{
			.name = "tkn_id",
		...
		{
			/* number of times token appears in the pattern */
			.name = "tkn_count",
			.type = SOS_TYPE_UINT64,
		},
		{
...
typedef struct  __attribute__ ((__packed__)) tkn_hist_value_s {
	uint32_t bin_width;
	uint32_t epoch;
	uint64_t tkn_id;
	uint64_t count;
...
typedef struct  __attribute__ ((__packed__)) ptn_hist_value_s {
	uint32_t bin_width;
	uint32_t epoch;
	uint64_t ptn_id;
	uint64_t count;
...
typedef struct  __attribute__ ((__packed__)) comp_hist_value_s {
	uint32_t bin_width;
	uint32_t epoch;
	uint64_t comp_id;
	uint64_t ptn_id;
...
{
	const char *sess_file = getenv("BALERD_SESSION_FILE");
	if (!sess_file) {
		fprintf(stdout, "The BALERD_SESSION_FILE environment variable must be set\n");
		exit(1);
	}
	g_session = dsos_session_open(sess_file);
	if (!g_session) {
		fprintf(stdout, "Error %d opening the session from file %s\n",
			errno, sess_file);
		exit(1);
	}
}

static dsos_container_t create_container(dsos_session_t sess, const char *path, int o_mode)
{
	return dsos_container_open(sess, path, SOS_PERM_RW | SOS_PERM_CREAT, o_mode);
}

static int create_store(bstore_dsos_t bs, const char *path, int o_mode)
{
	sos_schema_t schema;
	dsos_res_t res;
	int rc = ENOMEM;
	...
	bs->dict_sos = create_container(bs->sess, cpath, o_mode);
	if (!bs->dict_sos)
		goto err_1;

	schema = sos_schema_from_template(&token_value_schema);
	bs->token_value_schema = dsos_schema_create(bs->dict_sos, schema, &res);
	...
	bs->ptn_sos = create_container(bs->sess, cpath, o_mode);
	if (!bs->ptn_sos)
		goto err_2;
	schema = sos_schema_from_template(&pattern_schema);
	bs->pattern_schema = dsos_schema_create(bs->ptn_sos, schema, &res);
	...
	bs->msg_sos = create_container(bs->sess, cpath, o_mode);
	if (!bs->msg_sos)
		goto err_3;
	schema = sos_schema_from_template(&message_schema);
	bs->message_schema = dsos_schema_create(bs->msg_sos, schema, &res);
	...
	bs->ptn_tkn_sos = create_container(bs->sess, cpath, o_mode);
	if (!bs->ptn_tkn_sos)
		goto err_4;
	schema = sos_schema_from_template(&pattern_token_schema);
	bs->pattern_token_schema = dsos_schema_create(bs->ptn_tkn_sos, schema, &res);
	...
	bs->hist_sos = create_container(bs->sess, cpath, o_mode);
	if (!bs->hist_sos)
		goto err_5;
	schema = sos_schema_from_template(&token_hist_schema);
	bs->token_hist_schema = dsos_schema_create(bs->hist_sos, schema, &res);
	if (!bs->token_hist_schema)
		goto err_6;
	schema = sos_schema_from_template(&pattern_hist_schema);
	bs->pattern_hist_schema = dsos_schema_create(bs->hist_sos, schema, &res);
	if (!bs->pattern_hist_schema)
		goto err_6;
	schema = sos_schema_from_template(&component_hist_schema);
	bs->component_hist_schema = dsos_schema_create(bs->hist_sos, schema, &res);
	...
	bs->attr_sos = create_container(bs->sess, cpath, o_mode);
	if (!bs->attr_sos)
		goto err_6;
	schema = sos_schema_from_template(&attribute_schema);
	bs->attribute_schema = dsos_schema_create(bs->attr_sos, schema, &res);
	if (!bs->attribute_schema)
		goto err_7;
	schema = sos_schema_from_template(&pattern_attribute_schema);
	bs->pattern_attribute_schema = dsos_schema_create(bs->attr_sos, schema, &res);
	if (!bs->pattern_attribute_schema)
		goto err_7;

	free(schema);
	free(cpath);
	dsos_container_close(bs->attr_sos);
	dsos_container_close(bs->hist_sos);
	dsos_container_close(bs->ptn_tkn_sos);
	dsos_container_close(bs->msg_sos);
	dsos_container_close(bs->ptn_sos);
	dsos_container_close(bs->dict_sos);
	return 0;

	// TODO: Free the dschema in the bs
 err_7:
	dsos_container_close(bs->attr_sos);
 err_6:
	dsos_container_close(bs->hist_sos);
...
static bstore_t bs_open(bstore_plugin_t plugin, const char *path,
			int flags, int o_mode)
{
	...
	sprintf(cpath, "%s/Dictionary", path);
 reopen:
	perm = (flags & O_RDWR ? SOS_PERM_RW : SOS_PERM_RD);
	bs->dict_sos = dsos_container_open(bs->sess, cpath, perm, 0664);
	if (!bs->dict_sos) {
		if (errno != ENOENT)
			goto err_3;
		if (0 == (flags & O_CREAT))
			goto err_3;
		rc = create_store(bs, path, o_mode);
		if (rc)
			goto err_3;
		create = 1;
		flags &= ~O_CREAT;
	...
	bs->token_value_schema = dsos_schema_by_name(bs->dict_sos, "TokenValue");
	if (!bs->token_value_schema)
		goto err_4;
	bs->tkn_id_attr = dsos_schema_attr_by_name(bs->token_value_schema, "tkn_id");
	if (!bs->tkn_id_attr)
		goto err_4;
	bs->tkn_type_mask_attr =
		dsos_schema_attr_by_name(bs->token_value_schema, "tkn_type_mask");
	if (!bs->tkn_type_mask_attr)
		goto err_4;
	bs->tkn_text_attr = dsos_schema_attr_by_name(bs->token_value_schema, "tkn_text");
	if (!bs->tkn_text_attr)
		goto err_4;
	bs->tkn_id_iter = dsos_iter_create(bs->dict_sos, bs->token_value_schema, "tkn_id");
	if (!bs->tkn_id_iter)
		goto err_4;
	bs->tkn_text_iter = dsos_iter_create(bs->dict_sos, bs->token_value_schema, "tkn_text");
	if (!bs->tkn_id_iter)
		goto err_4;
	/*
	 ...
	 */
	sprintf(cpath, "%s/Messages", path);
	bs->msg_sos = dsos_container_open(bs->sess, cpath, SOS_PERM_RW, 0664);
	if (!bs->msg_sos)
		goto err_4;
	bs->message_schema = dsos_schema_by_name(bs->msg_sos, "Message");
	if (!bs->message_schema)
		goto err_5;
	bs->pt_key_attr = dsos_schema_attr_by_name(bs->message_schema, "pt_key");
	if (!bs->pt_key_attr)
		goto err_5;
	bs->ct_key_attr = dsos_schema_attr_by_name(bs->message_schema, "ct_key");
	if (!bs->pt_key_attr)
		goto err_5;
	bs->tc_key_attr = dsos_schema_attr_by_name(bs->message_schema, "tc_key");
	if (!bs->tc_key_attr)
		goto err_5;
	bs->tkn_ids_attr = dsos_schema_attr_by_name(bs->message_schema, "tkn_ids");
	if (!bs->tkn_ids_attr)
		goto err_5;


	sprintf(cpath, "%s/Patterns", path);
	bs->ptn_sos = dsos_container_open(bs->sess, cpath, SOS_PERM_RW, 0664);
	if (!bs->ptn_sos)
		goto err_5;
	bs->pattern_schema = dsos_schema_by_name(bs->ptn_sos, "Pattern");
	if (!bs->pattern_schema)
		goto err_6;
	bs->ptn_id_attr = dsos_schema_attr_by_name(bs->pattern_schema, "ptn_id");
	if (!bs->ptn_id_attr)
		goto err_6;
	bs->first_seen_attr = dsos_schema_attr_by_name(bs->pattern_schema, "first_seen");
	if (!bs->first_seen_attr)
		goto err_6;
	bs->last_seen_attr = dsos_schema_attr_by_name(bs->pattern_schema, "last_seen");
	if (!bs->last_seen_attr)
		goto err_6;
	bs->tkn_type_ids_attr =
		dsos_schema_attr_by_name(bs->pattern_schema, "tkn_type_ids");
	if (!bs->tkn_type_ids_attr)
		goto err_6;
	bs->ptn_iter = dsos_iter_create(bs->ptn_sos,
					bs->pattern_schema,
					"ptn_id");
	if (!bs->ptn_iter)
		goto err_4;
	...
	sprintf(cpath, "%s/PatternTokens", path);
	bs->ptn_tkn_sos = dsos_container_open(bs->sess, cpath, SOS_PERM_RW, 0664);
	if (!bs->ptn_tkn_sos)
		goto err_6;
	bs->pattern_token_schema = dsos_schema_by_name(bs->ptn_tkn_sos, "PatternToken");
	if (!bs->pattern_token_schema)
		goto err_7;
	bs->ptn_pos_tkn_key_attr =
		dsos_schema_attr_by_name(bs->pattern_token_schema, "ptn_pos_tkn_key");
	if (!bs->ptn_pos_tkn_key_attr)
		goto err_7;
	bs->ptn_pos_tkn_iter =
		dsos_iter_create(bs->ptn_tkn_sos, bs->pattern_token_schema,
				 "ptn_pos_tkn_key");
	if (!bs->ptn_pos_tkn_iter)
		goto err_7;

	sprintf(cpath, "%s/History", path);
	bs->hist_sos = dsos_container_open(bs->sess, cpath, SOS_PERM_RW, 0664);
	if (!bs->hist_sos)
		goto err_7;
	bs->token_hist_schema = dsos_schema_by_name(bs->hist_sos, "TokenHist");
	if (!bs->token_hist_schema)
		goto err_8;
	bs->tkn_hist_key_attr =
		dsos_schema_attr_by_name(bs->token_hist_schema, "tkn_hist_key");
	if (!bs->tkn_hist_key_attr)
		goto err_8;

	bs->pattern_hist_schema = dsos_schema_by_name(bs->hist_sos, "PatternHist");
	if (!bs->pattern_hist_schema)
		goto err_8;
	bs->ptn_hist_key_attr =
		dsos_schema_attr_by_name(bs->pattern_hist_schema, "ptn_hist_key");
	if (!bs->ptn_hist_key_attr)
		goto err_8;

	bs->component_hist_schema = dsos_schema_by_name(bs->hist_sos, "ComponentHist");
	if (!bs->component_hist_schema)
		goto err_8;
	bs->comp_hist_key_attr =
		dsos_schema_attr_by_name(bs->component_hist_schema, "comp_hist_key");
	if (!bs->comp_hist_key_attr)
		goto err_8;

	sprintf(cpath, "%s/Attribute", path);
	bs->attr_sos = dsos_container_open(bs->sess, cpath, SOS_PERM_RW, 0664);
	if (!bs->attr_sos)
		goto err_8;

	bs->attribute_schema = dsos_schema_by_name(bs->attr_sos, "Attribute");
	if (!bs->attribute_schema)
		goto err_9;
	bs->attr_type_attr = dsos_schema_attr_by_name(bs->attribute_schema,
						     "type");
	if (!bs->attr_type_attr)
		goto err_9;
	bs->pattern_attribute_schema =
			dsos_schema_by_name(bs->attr_sos, "PatternAttribute");
	if (!bs->pattern_attribute_schema)
		goto err_9;
	bs->pa_at_attr = dsos_schema_attr_by_name(
			bs->pattern_attribute_schema, "attr_type");
	if (!bs->pa_at_attr)
		goto err_9;
	bs->pa_av_attr = dsos_schema_attr_by_name(
			bs->pattern_attribute_schema, "attr_value");
	if (!bs->pa_av_attr)
		goto err_9;
	bs->pa_ptv_attr = dsos_schema_attr_by_name(
			bs->pattern_attribute_schema, "ptn_type_value");
	if (!bs->pa_ptv_attr)
		goto err_9;
	bs->pa_tv_attr = dsos_schema_attr_by_name(
			bs->pattern_attribute_schema, "type_value");
	if (!bs->pa_tv_attr)
		goto err_9;
	bs->pa_tp_attr = dsos_schema_attr_by_name(
	...
	dsos_iter_t diter = dsos_iter_create(bs->dict_sos,
					     bs->token_value_schema, "tkn_id");

	sos_obj_t last = dsos_iter_end(diter);
	if (!last) {
		...
		if (bs->next_tkn_id < 0x0100)
			/* The last token is a type */
			bs->next_tkn_id = 0x0100;
		sos_obj_put(last);
	}
	diter = dsos_iter_create(bs->ptn_sos, bs->pattern_schema, "ptn_id");
	if (!diter)
		goto err_9;
	last = dsos_iter_end(diter);
	if (!last) {
...
{
	bstore_dsos_t bss = (bstore_dsos_t)bs;
	if (!bs)
		return;
	free(bs->path);
	dsos_container_close(bss->attr_sos);
	dsos_container_close(bss->dict_sos);
	dsos_container_close(bss->ptn_sos);
	dsos_container_close(bss->ptn_tkn_sos);
	dsos_container_close(bss->msg_sos);
	dsos_container_close(bss->hist_sos);
	free(bs);
}
 ...
 */
static btkn_id_t allocate_tkn_id(bstore_dsos_t bss)
{
...
static bptn_id_t allocate_ptn_id(bstore_dsos_t bss)
{
...
static int __add_tkn_with_id(bstore_dsos_t bss, btkn_t tkn, uint64_t count)
{
	int rc;
	...
	dsos_transaction_begin(bss->dict_sos, NULL);

	/* Allocate a new object */
	tkn_obj = dsos_obj_new_size(bss->token_value_schema, tkn->tkn_str->blen + 512);
	if (!tkn_obj) {
		rc = errno;
		goto err_0;
	}
	...
	rc = dsos_obj_create(bss->dict_sos, NULL, bss->token_value_schema, tkn_obj);
	if (rc)
		goto err_1;
	sos_obj_put(tkn_obj);
	rc = 0;
	if (bss->next_tkn_id <= tkn->tkn_id) {
		bss->next_tkn_id = tkn->tkn_id + 1;
	}
	goto out;

 err_1:
	// TODO: sos_obj_delete(tkn_obj);
	sos_obj_put(tkn_obj);
 err_0:
 out:
	dsos_transaction_end(bss->dict_sos);
	return rc;
}
				     ...
				     int found, void *arg)
{
	...
	if (found) {
		tkn_obj = sos_ref_as_obj(dsos_schema_local(ctxt->bss->dict_sos), *ref);
		if (!tkn_obj) {
			OOM();
			goto err_0;
		}
	...
	/* Allocate a new object */
	tkn_obj = sos_obj_new_with_array_reserve(
				dsos_schema_local(ctxt->bss->token_value_schema),
				sz + 512);
	if (!tkn_obj) {
		OOM();
		goto err_0;
...
static btkn_id_t bs_tkn_add(bstore_t bs, btkn_t tkn)
{
	...
	tkn_obj = dsos_index_find(bss->dict_sos, bss->token_value_schema,
			bss->tkn_text_attr, text_key);
	if (tkn_obj) {
		/* Update the token value */
		tkn_value = sos_obj_ptr(tkn_obj);
		tkn_value->tkn_count++;
		tkn_value->tkn_type_mask |= tkn->tkn_type_mask;
		/* Update the memory tkn */
		tkn->tkn_id = tkn_value->tkn_id;
		tkn->tkn_type_mask = tkn_value->tkn_type_mask;
		sos_obj_put(tkn_obj);
		tkn_id = tkn->tkn_id;
		rc = dsos_obj_update(bss->dict_sos, tkn_obj);
		goto out;
	}

	/* Allocate a new object */
	sz = tkn->tkn_str->blen + 1;
	tkn_obj = dsos_obj_new_size(bss->token_value_schema, sz + 512);
	if (!tkn_obj) {
		OOM();
		tkn_obj = 0;
		goto out;
	...
	tkn_value = sos_obj_ptr(tkn_obj);
	tkn_value->tkn_count = tkn->tkn_count;
	...
	v->data->array.data.byte_[tkn->tkn_str->blen] = '\0';
	sos_value_put(v);
	...
	if (btkn_has_type(tkn, BTKN_TYPE_WORD)
		...
		tkn->tkn_type_mask &= ~BTKN_TYPE_MASK(BTKN_TYPE_TEXT);
	tkn_value->tkn_type_mask = tkn->tkn_type_mask;
	tkn_id = allocate_tkn_id(bss);
	tkn->tkn_id = tkn_value->tkn_id = tkn_id;

	rc = dsos_obj_create(bss->dict_sos, NULL, bss->token_value_schema, tkn_obj);
	if (rc) {
		printf("Shitstorm 1.0\n");
	}
	sos_obj_put(tkn_obj);
 out:
...
static int bs_tkn_add_with_id(bstore_t bs, btkn_t tkn)
{
	...
	tkn_obj = dsos_index_find(bss->dict_sos,
				  bss->token_value_schema,
				  bss->tkn_text_attr, text_key);
	if (tkn_obj) {
		sos_obj_put(tkn_obj);
		rc = EEXIST;
	...
	*id_key->as.uint64 = tkn->tkn_id;
	tkn_obj = dsos_index_find(bss->dict_sos,
				  bss->token_value_schema,
				  bss->tkn_id_attr, id_key);
	if (tkn_obj) {
		sos_obj_put(tkn_obj);
		rc = EEXIST;
		goto err_0;
...
static int __iter_init(bsos_iter_t iter, int type)
{
	...
	bstore_dsos_t bss = (void*)iter->bs;
	...
	case BTKN_ITER:
		if (type != TKN_ITER)
			return EINVAL;
		cont = bss->dict_sos;
		schema = bss->token_value_schema;
		attr_name = "tkn_id";
		break;
	case BMSG_ITER:
		cont = bss->msg_sos;
		schema = bss->message_schema;
		switch (type) {
	...
	case BPTN_TKN_ITER:
		if (type != PTN_TKN_ITER)
			return EINVAL;
		cont = bss->ptn_tkn_sos;
		schema = bss->pattern_token_schema;
		attr_name = "ptn_pos_tkn_key";
		break;
	case BTKN_HIST_ITER:
		if (type != TKN_HIST_ITER)
			return EINVAL;
		cont = bss->hist_sos;
		schema = bss->token_hist_schema;
		attr_name = "tkn_hist_key";
		break;
	case BPTN_HIST_ITER:
		if (type != PTN_HIST_ITER)
			return EINVAL;
		cont = bss->hist_sos;
		schema = bss->pattern_hist_schema;
		attr_name = "ptn_hist_key";
		break;
	case BCOMP_HIST_ITER:
		if (type != COMP_HIST_ITER)
			return EINVAL;
		cont = bss->hist_sos;
		schema = bss->component_hist_schema;
		attr_name = "comp_hist_key";
		break;
	default:
		return EINVAL;
	}
	iter->iter = dsos_iter_create(cont, schema, attr_name);
	if (!iter->iter)
		return errno;
	return 0;
}
...
static btkn_iter_t bs_tkn_iter_new(bstore_t bs)
{
	...
	if (ti) {
		...
		ti->iter = dsos_iter_create(bss->dict_sos,
					    bss->token_value_schema,
					    "tkn_id");
		if (!ti->iter)
			goto err;
...
static void bs_tkn_iter_free(btkn_iter_t i)
{
	...
	if (ti->obj)
		sos_obj_put(ti->obj);
	if (ti->iter)
		dsos_iter_delete(ti->iter);
	free(i);
}

static uint64_t __iter_card(bsos_iter_t iter)
{
	dsos_iter_stats_t stats;
	if (!iter)
		return 0;
	stats = dsos_iter_stats(iter->iter);
	return stats.cardinality;
}

static uint64_t bs_tkn_iter_card(btkn_iter_t i)
{
	return __iter_card((bsos_iter_t)i);
}

static btkn_t __make_tkn(bstore_dsos_t bss, sos_obj_t tkn_obj)
{
	struct sos_value_s v_, *v;
	tkn_value_t tv;
	btkn_t tkn;
...
static int bs_tkn_iter_first(btkn_iter_t iter)
{
	...
	i->obj = dsos_iter_begin(i->iter);
	return (i->obj ? 0 : ENOENT);
}

static btkn_t bs_tkn_iter_obj(btkn_iter_t iter)
{
	bsos_iter_t i = (bsos_iter_t)iter;
	bstore_dsos_t bss = (bstore_dsos_t)i->bs;
	return __make_tkn(bss, i->obj);
}

static int bs_tkn_iter_next(btkn_iter_t iter)
{
	bsos_iter_t i = (bsos_iter_t)iter;
	i->obj = dsos_iter_next(i->iter);
	return (i->obj ? 0 : ENOENT);
}
...
static bptn_iter_t bs_ptn_iter_new(bstore_t bs)
{
	...
	pi->iter = dsos_iter_create(bss->ptn_sos, bss->pattern_schema, "last_seen");
	if (!pi->iter) {
		free(pi);
		return NULL;
	}
...
static bptn_t __make_ptn(bstore_dsos_t bss, bsos_iter_t i, sos_obj_t ptn_obj)
{
	...
	ptn->first_seen.tv_sec = sptn->first_seen.fine.secs;
	ptn->first_seen.tv_usec = sptn->first_seen.fine.usecs;
	ptn->last_seen.tv_sec = sptn->last_seen.fine.secs;
	ptn->last_seen.tv_usec = sptn->last_seen.fine.usecs;
...
static bptn_t bs_ptn_find(bstore_t bs, bptn_id_t ptn_id)
{
	...
	ptn_obj = dsos_index_find(bss->ptn_sos,
				bss->pattern_schema,
				bss->ptn_id_attr, ptn_key);
	sos_key_put(ptn_key);
	if (!ptn_obj)
		goto out;
...
static int bs_ptn_find_by_ptnstr(bstore_t bs, bptn_t ptn)
{
	...
	ptn->first_seen.tv_sec = ptn_value->first_seen.fine.secs;
	ptn->first_seen.tv_usec = ptn_value->first_seen.fine.usecs;
	ptn->last_seen.tv_sec = ptn_value->last_seen.fine.secs;
	ptn->last_seen.tv_usec = ptn_value->last_seen.fine.usecs;
	sos_obj_put(ptn_obj);
	rc = 0;
...
static int __matching_ptn(bptn_iter_t iter, int fwd, sos_obj_t obj)
{
	bsos_iter_t i = (bsos_iter_t)iter;
	ptn_t ptn;
	struct timeval tv;
	...
	for (; obj; obj = fwd ? dsos_iter_next(i->iter) : dsos_iter_prev(i->iter)) {
...
static int __bs_ptn_iter_find(bptn_iter_t iter, int fwd, const struct timeval *tv)
{
	SOS_KEY(key);
	ods_key_value_t kv = key->as.ptr;
	union sos_timestamp_u *sos_ts;
	bsos_iter_t i = (bsos_iter_t)iter;

	sos_ts = (void*)kv->value;
	kv->len = sizeof(sos_ts->fine);

	if (tv) {
		sos_ts->fine.secs = tv->tv_sec;
		sos_ts->fine.usecs = tv->tv_usec;
	} else {
		if (fwd) {
			sos_ts->fine.secs = 0;
			sos_ts->fine.usecs = 0;
	...
	if (fwd) {
		i->obj = dsos_iter_find_ge(i->iter, key);
	} else {
		i->obj = dsos_iter_find_le(i->iter, key);
	}
	if (!i->obj)
		return ENOENT;
	return __matching_ptn(iter, fwd, i->obj);
}

static int bs_ptn_iter_find_fwd(bptn_iter_t iter, const struct timeval *tv)
{
	return __bs_ptn_iter_find(iter, 1, tv);
}

static int bs_ptn_iter_find_rev(bptn_iter_t iter, const struct timeval *tv)
{
	return __bs_ptn_iter_find(iter, 0, tv);
}

static int bs_ptn_iter_first(bptn_iter_t iter)
{
	return __bs_ptn_iter_find(iter, 1, NULL);
}

static int bs_ptn_iter_last(bptn_iter_t iter)
{
	return __bs_ptn_iter_find(iter, 0, NULL);
}
...
static int bs_ptn_iter_next(bptn_iter_t iter)
{
	...
	if (!i->obj)
		return ENOENT;
	return __matching_ptn(iter, 1, i->obj);
}

static int bs_ptn_iter_prev(bptn_iter_t iter)
{
	bsos_iter_t i = (bsos_iter_t)iter;
	i->obj = dsos_iter_prev(i->iter);
	if (!i->obj)
		return ENOENT;
	return __matching_ptn(iter, 0, i->obj);
}
...
static bptn_tkn_iter_t bs_ptn_tkn_iter_new(bstore_t bs)
{
	...
	if (pti) {
		...
		pti->iter = dsos_iter_create(bss->ptn_tkn_sos,
					     bss->pattern_token_schema,
					     "ptn_pos_tkn_key");
		if (!pti->iter)
			goto err;
	}
...
static bmsg_t __make_msg(bstore_dsos_t bss, bsos_iter_t i, sos_obj_t msg_obj)
{
...
static sos_obj_t __next_matching_msg(bsos_iter_t i, int forwards, sos_obj_t obj)
{
	msg_t msg;
	struct timeval tv;
	sos_obj_t (*step)(dsos_iter_t);
	step = forwards?dsos_iter_next:dsos_iter_prev;

	for (; obj; obj = step(i->iter)) {
		     ...
		     bcomp_id_t comp_id, bptn_id_t ptn_id)
{
	...
	if (!i->iter) {
		...
		i->iter = dsos_iter_create(bss->msg_sos, bss->message_schema, "tc_key");
		if (!i->iter) {
			rc = errno;
			goto err;
		}
	...
	if (fwd) {
		obj = dsos_iter_find_ge(i->iter, msg_key);
	} else {
		obj = dsos_iter_find_le(i->iter, msg_key);
	}
	i->obj = __next_matching_msg(i, fwd, obj);
	if (!i->obj) {
		rc = ENOENT;
		goto err;
	}
...
static int bs_msg_iter_next(bmsg_iter_t iter)
{
	...
	if (!i->obj)
		return ENOENT;
	i->obj = __next_matching_msg(i, 1, i->obj);
	if (i->obj)
		return 0;
	return ENOENT;
}
...
{
	bsos_iter_t i = (bsos_iter_t)iter;
	sos_obj_t obj = dsos_iter_prev(i->iter);
	if (!obj)
		return ENOENT;
	i->obj = __next_matching_msg(i, 0, obj);
	if (i->obj)
		return 0;
	return ENOENT;
}
...
int bs_msg_iter_filter_set(bmsg_iter_t iter, bstore_iter_filter_t filter)
{
	...
	i->iter = dsos_iter_create(bss->msg_sos, bss->message_schema, sos_attr_name(attr));
	if (!i->iter) {
		return errno;
	}
out:
...
static bptn_id_t bs_ptn_add(bstore_t bs, struct timeval *tv, bstr_t ptn)
{
	...
	tkn_count = ptn->blen / sizeof(ptn->u64str[0]);
	ptn_size = encode_ptn(ptn, tkn_count);
	sos_key_set(ptn_key, ptn->cstr, ptn_size);

	if (bstore_lock)
		pthread_mutex_lock(&bss->ptn_lock);

	dsos_transaction_begin(bss->ptn_sos, NULL);

	obj = dsos_index_find(bss->ptn_sos, bss->pattern_schema,
			bss->tkn_type_ids_attr, ptn_key);

	if (obj)
		goto update_routine;

	/* 'create' routine */
	obj = dsos_obj_new_size(bss->pattern_schema, ptn_size + 512);
	if (!obj)
		goto out;
	ptn_value = sos_obj_ptr(obj);
	ptn_value->first_seen.fine.secs = ptn_value->last_seen.fine.secs = tv->tv_sec;
	ptn_value->first_seen.fine.usecs = ptn_value->last_seen.fine.usecs = tv->tv_usec;
	ptn_value->tkn_count = tkn_count;
	ptn_value->count = 1;

	sos_value_t v;
	struct sos_value_s v_;
	v = sos_array_new(&v_, bss->tkn_type_ids_attr,
			  obj, ptn_size);
	sos_value_memcpy(v, ptn->cstr, ptn_size);
	sos_value_put(v);
	ptn_id = ptn_value->ptn_id = allocate_ptn_id(bss);
	rc = dsos_obj_create(bss->ptn_sos, NULL, bss->pattern_schema, obj);
	if (rc) {
		ptn_id = 0;
	}
 ...
 update_routine:
	...
	first_seen.tv_sec = ptn_value->first_seen.fine.secs;
	first_seen.tv_usec = ptn_value->first_seen.fine.usecs;
	if (timercmp(tv, &first_seen, <)) {
		ptn_value->first_seen.fine.secs = tv->tv_sec;
		ptn_value->first_seen.fine.usecs = tv->tv_usec;
	}
	last_seen.tv_sec = ptn_value->last_seen.fine.secs;
	last_seen.tv_usec = ptn_value->last_seen.fine.usecs;
	if (timercmp(&last_seen, tv, <)) {
		ptn_value->last_seen.fine.secs = tv->tv_sec;
		ptn_value->last_seen.fine.usecs = tv->tv_usec;
	}
	ptn_value->count += 1;
	dsos_obj_update(bss->ptn_sos, obj);

 out:
	if (obj)
		sos_obj_put(obj);
	dsos_transaction_end(bss->ptn_sos);
	if (ptn_key != stack_key)
		sos_key_put(ptn_key);
	if (bstore_lock)
		pthread_mutex_unlock(&bss->ptn_lock);
			  ...
			  btkn_id_t tkn_id)
{
	...
	bstore_dsos_t bss = (bstore_dsos_t)bs;
	sos_obj_t obj;
	ptn_tkn_value_t hval;
	int rc;

	sos_key_join(key, bss->ptn_pos_tkn_key_attr, ptn_id, tkn_pos, tkn_id);

	dsos_transaction_begin(bss->ptn_tkn_sos, NULL);

	obj = dsos_index_find(bss->ptn_tkn_sos, bss->pattern_token_schema,
			      bss->ptn_pos_tkn_key_attr, key);
	if (!obj)
		goto new_hist;

	/* update routine */
	hval = sos_obj_ptr(obj);
	hval->tkn_count += 1;
	rc = dsos_obj_update(bss->hist_sos, obj);
	goto out;

 new_hist:
	/* create routine */
	obj = dsos_obj_new(bss->pattern_token_schema);
	if (!obj) {
		rc = errno;
		goto out;
	}
	...
	rc = dsos_obj_create(bss->ptn_tkn_sos, NULL, bss->pattern_token_schema, obj);

 out:
	if (obj)
		sos_obj_put(obj);
	dsos_transaction_end(bss->ptn_tkn_sos);
	return rc;
}
			      ...
			      btkn_id_t tkn_id)
{
	...
	ptn_tkn_obj = dsos_index_find(bss->ptn_tkn_sos,
				      bss->pattern_token_schema,
				      bss->ptn_pos_tkn_key_attr, key);
	if (ptn_tkn_obj) {
		ptn_tkn_value_t ptn_tkn = sos_obj_ptr(ptn_tkn_obj);
		tkn = bs_tkn_find_by_id(bs, tkn_id);
		if (!tkn) {
			errno = ENOMEM;
			goto out;
		}
		tkn->tkn_count = ptn_tkn->tkn_count;
	} else {
		tkn = NULL;
	}
 out:
...
static int bs_comp_hist_update(bstore_t bs, bptn_id_t ptn_id,
			       bcomp_id_t comp_id,
			       time_t secs, time_t bin_width)
{
	...
	bstore_dsos_t bss = (bstore_dsos_t)bs;
	sos_obj_t obj;
	comp_hist_value_t hval;
	int rc;

	sos_key_join(key, bss->comp_hist_key_attr, bin_width, secs, comp_id, ptn_id);

	dsos_transaction_begin(bss->hist_sos, NULL);

	obj = dsos_index_find(bss->hist_sos, bss->component_hist_schema,
			      bss->comp_hist_key_attr, key);
	if (!obj)
		goto new_hist;
 ...
 new_hist:
	/* create routine */
	obj = dsos_obj_new(bss->component_hist_schema);
	if (!obj) {
		rc = errno;
		goto out;
	}
	hval = sos_obj_ptr(obj);
	hval->bin_width = bin_width;
	hval->epoch = secs;
	hval->comp_id = comp_id;
	hval->ptn_id = ptn_id;
	hval->count = 1;

	rc = dsos_obj_create(bss->hist_sos, NULL, bss->component_hist_schema, obj);

 out:
	if (obj)
		sos_obj_put(obj);
	dsos_transaction_end(bss->hist_sos);
	return rc;
}

/* Update to ptn_hist and comp_hist */
static int bs_ptn_hist_update(bstore_t bs, bptn_id_t ptn_id, bcomp_id_t comp_id,
			      time_t secs, time_t bin_width)
{
	SOS_KEY(key);
	bstore_dsos_t bss = (bstore_dsos_t)bs;
	sos_obj_t obj;
	ptn_hist_value_t hval;
	int rc;

	sos_key_join(key, bss->ptn_hist_key_attr, bin_width, secs, ptn_id);

	/* do the ptn_hist update first */
	dsos_transaction_begin(bss->hist_sos, NULL);

	obj = dsos_index_find(bss->hist_sos, bss->pattern_hist_schema,
			      bss->ptn_hist_key_attr, key);
	if (!obj)
		goto new_hist;
 ...
 new_hist:
	/* create routine */
	obj = dsos_obj_new(bss->pattern_hist_schema);
	if (!obj) {
		rc = errno;
		goto out;
	}
	...
	rc = dsos_obj_create(bss->hist_sos, NULL, bss->pattern_hist_schema, obj);

 out:
	if (obj)
		sos_obj_put(obj);
	dsos_transaction_end(bss->hist_sos);
	if (rc == 0) { /* if there was an error, no need to update comp_hist */
		rc = bs_comp_hist_update(bs, ptn_id, comp_id, secs, bin_width);
	}
	return rc;
}

static int bs_tkn_hist_update(bstore_t bs, time_t secs, time_t bin_width,
			      btkn_id_t tkn_id)
{
	SOS_KEY(key);
	bstore_dsos_t bss = (bstore_dsos_t)bs;
	sos_obj_t obj;
	tkn_hist_value_t hval;
	int rc;
	...
	obj = dsos_index_find(bss->hist_sos, bss->token_hist_schema,
			      bss->tkn_hist_key_attr, key);
	if (!obj)
		goto new_hist;
 ...
 new_hist:
	/* create routine */
	obj = dsos_obj_new(bss->token_hist_schema);
	if (!obj) {
		rc = errno;
		goto out;
	}
	...
	rc = dsos_obj_create(bss->hist_sos, NULL, bss->token_hist_schema, obj);

 out:
	if (obj)
		sos_obj_put(obj);
	dsos_transaction_end(bss->hist_sos);
	return rc; // sos_index_visit(idx, key, hist_cb, NULL);
}
...
static int bs_msg_add(bstore_t bs, struct timeval *tv, bmsg_t msg)
{
	...
	msg_obj = dsos_obj_new_size(bss->message_schema, bmsg_sz + 512);
	if (!msg_obj)
		goto err_0;
	...
	dsos_transaction_begin(bss->msg_sos, NULL);
	rc = dsos_obj_create(bss->msg_sos, NULL, bss->message_schema, msg_obj);
	dsos_transaction_end(bss->msg_sos);
	if (rc)
		goto err_1;
	sos_obj_put(msg_obj);
	if (bstore_lock)
...
static int __ptn_tkn_iter_check(bsos_iter_t i)
{
	...
	if (!i->obj)
		return ENOENT;
	val = sos_obj_ptr(i->obj);
	if ((val->pos != i->filter.tkn_pos) || (val->ptn_id != i->filter.ptn_id))
		return ENOENT;
	return 0;
}
...
static btkn_t make_ptn_tkn(bsos_iter_t i)
{
	...
	tkn = bs_tkn_find_by_id(i->bs, val->tkn_id);
	/* set the 'count' to ptn_tkn_pos count */
	if (tkn) {
		tkn->tkn_count = val->tkn_count;
	}
	return tkn;
...
static int bs_ptn_tkn_iter_first(bptn_tkn_iter_t iter)
{
	...
	sos_key_join(key, bss->ptn_pos_tkn_key_attr,
...
{
	bstore_dsos_t bss = (bstore_dsos_t)iter->bs;
	bsos_iter_t i = (bsos_iter_t)iter;
	SOS_KEY(key);
	int rc;
...
{
	bsos_iter_t mi = calloc(1, sizeof(*mi));
	bstore_dsos_t bss = (void*)bs;
	if (!mi)
		return NULL;
	mi->bs = bs;
	mi->biter_type = BTKN_HIST_ITER;
	mi->iter_type = TKN_HIST_ITER;
	mi->iter = dsos_iter_create(bss->hist_sos, bss->token_hist_schema,
				    "tkn_hist_key");
	if (!mi->iter) {
		free(mi);
		return NULL;
...
{
	tkn_hist_value_t hval;
	sos_key_t key_o = dsos_iter_key(i->iter, i->obj);
	time_t start_time = i->filter.tv_begin.tv_sec;
	time_t end_time = i->filter.tv_end.tv_sec;
	for ( ; i->obj; i->obj = dsos_iter_next(i->iter)) {
		key_o = dsos_iter_key(i->iter, i->obj);
		hval = sos_obj_ptr(i->obj);

		sos_key_put(key_o);

		if (i->filter.bin_width && (i->filter.bin_width != hval->bin_width))
			/* Bin width doesn't match, no more matches */
			break;

		if ((start_time && hval->epoch < start_time)
		    || (end_time && end_time < hval->epoch))
			/* Time doesn't match, no more matches */
			break;

		if (i->filter.tkn_id && (i->filter.tkn_id != hval->tkn_id)) {
...
{
	bsos_iter_t i = (bsos_iter_t)iter;
	tkn_hist_value_t val;
	if (!i->obj)
		return NULL;
	val = sos_obj_ptr(i->obj);
	tkn_h->bin_width = val->bin_width;
	tkn_h->time      = val->epoch;
	tkn_h->tkn_id    = val->tkn_id;
	tkn_h->tkn_count = val->count;
	return tkn_h;
}
...
static int bs_tkn_hist_iter_next(btkn_hist_iter_t iter)
{
	...
	/* step to next obj */
	i->obj = dsos_iter_next(i->iter);
	return __tkn_hist_next(i); /* apply filter */
}
...
bptn_hist_iter_t bs_ptn_hist_iter_new(bstore_t bs)
{
	...
	if (!mi)
		return NULL;
	mi->bs = bs;
	mi->biter_type = BPTN_HIST_ITER;
	mi->iter_type = PTN_HIST_ITER;
	mi->iter = dsos_iter_create(bss->hist_sos, bss->pattern_hist_schema,
				    "ptn_hist_key");
	if (!mi->iter) {
		free(mi);
		return NULL;
...
int __ptn_hist_next(bsos_iter_t i)
{
	...
	for ( ; i->obj; i->obj = dsos_iter_next(i->iter)) {
		...
		if (i->filter.ptn_id && (i->filter.ptn_id != hval->ptn_id)) {
			/* ptn id doesn't match, keep looking */
			sos_obj_put(i->obj);
			continue;
		}
...
__bs_ptn_hist_iter_find(bptn_hist_iter_t iter, int fwd, bptn_hist_t ptn_h)
{
	...
	sos_key_join(key, bss->ptn_hist_key_attr,
...
{
	bsos_iter_t i = (bsos_iter_t)iter;
	ptn_hist_value_t val;
	if (!i->obj)
		return NULL;
	val = sos_obj_ptr(i->obj);
	ptn_h->bin_width = val->bin_width;
	ptn_h->time      = val->epoch;
	ptn_h->ptn_id    = val->ptn_id;
	ptn_h->msg_count = val->count;
	return ptn_h;
}
...
static int bs_ptn_hist_iter_next(bptn_hist_iter_t iter)
{
	...
	/* step to next obj */
	i->obj = dsos_iter_next(i->iter);
	return __ptn_hist_next(i); /* apply filter */
}
...
bcomp_hist_iter_t bs_comp_hist_iter_new(bstore_t bs)
{
	...
	if (!mi)
		return NULL;
	mi->bs = bs;
	mi->biter_type = BCOMP_HIST_ITER;
	mi->iter_type = COMP_HIST_ITER;
	mi->iter = dsos_iter_create(bss->hist_sos, bss->component_hist_schema,
				    "comp_hist_key");
	if (!mi->iter) {
		free(mi);
		return NULL;
...
static int __comp_hist_next(bsos_iter_t i)
{
	...
	for ( ; i->obj; i->obj = dsos_iter_next(i->iter)) {
		...
		if (i->filter.comp_id && (i->filter.comp_id != hval->comp_id))
...
__bs_comp_hist_iter_find(bcomp_hist_iter_t iter, int fwd, bcomp_hist_t comp_h)
{
	...
	sos_key_join(key, bss->comp_hist_key_attr,
...
{
	bsos_iter_t i = (bsos_iter_t)iter;
	comp_hist_value_t val;
	if (!i->obj)
		return NULL;
	val = sos_obj_ptr(i->obj);
	comp_h->bin_width = val->bin_width;
	comp_h->time      = val->epoch;
	comp_h->comp_id   = val->comp_id;
	comp_h->ptn_id    = val->ptn_id;
	comp_h->msg_count = val->count;
	return comp_h;
}
...
static int bs_comp_hist_iter_next(bcomp_hist_iter_t iter)
{
	...
	/* step to next obj */
	i->obj = dsos_iter_next(i->iter);
	return __comp_hist_next(i); /* apply filter */
}
...
static int bs_attr_new(bstore_t bs, const char *attr_type)
{
	...
	if (bstore_lock)
		pthread_mutex_lock(&bss->ptn_lock);
	dsos_transaction_begin(bss->attr_sos, NULL);

	if (len > 2048) {
		text_key = sos_key_new(len);
		if (!text_key) {
			rc = ENOMEM;
			OOM();
			goto out;
	...
	encode_tkn_key(text_key, attr_type, len - 1);
	obj = dsos_index_find(bss->attr_sos, bss->attribute_schema,
			      bss->attr_type_attr, text_key);
	if (obj) {
		rc = EEXIST;
		goto out;
	}

	obj = dsos_obj_new_size(bss->attribute_schema, len);
	if (!obj) {
		rc = errno;
		goto out;
	}
	v = sos_array_new(&v_, bss->attr_type_attr, obj, len);
	if (!v) {
		rc = errno;
		goto out;
	}
	memcpy(v->data->array.data.byte_, attr_type, len);
	sos_value_put(v);

	rc = dsos_obj_create(bss->attr_sos, NULL, bss->attribute_schema, obj);
 out:
	if (obj)
		sos_obj_put(obj);
	if (text_key != stack_key) {
	...
	dsos_transaction_end(bss->attr_sos);
	if (bstore_lock)
		pthread_mutex_unlock(&bss->ptn_lock);
	return rc;
}
...
static int bs_ptn_attr_unset(bstore_t bs, bptn_id_t ptn_id, const char *attr_type);
		...
		const char *attr_type, const char *attr_value)
{
	...
	bstore_dsos_t bss = (bstore_dsos_t)bs;
	SOS_KEY_SZ(stack_key, 2048);
	sos_key_t key = stack_key;
	int t_len = strlen(attr_type) + 1;
	int v_len = strlen(attr_value) + 1;
	int o_len = t_len + v_len + 512;
	sos_obj_t obj = NULL;
	sos_value_t v;
	struct sos_value_s v_;
	ptn_attr_t val;
	...
	if (rc) /* no such attr_type, do not proceed */
		return rc;
	...
	obj = dsos_obj_new_size(bss->pattern_attribute_schema, o_len);
	if (!obj) {
		rc = errno;
		goto out;
	}
	...
	/* attr_type */
	v = sos_array_new(&v_, bss->pa_at_attr, obj, t_len);
	if (!v) {
		rc = errno;
		goto out;
	}
	memcpy(v->data->array.data.byte_, attr_type, t_len);
	sos_value_put(v);

	/* attr_value */
	v = sos_array_new(&v_, bss->pa_av_attr, obj, v_len);
	if (!v) {
		rc = errno;
		goto out;
	}
	memcpy(v->data->array.data.byte_, attr_value, v_len);
	sos_value_put(v);
	rc = dsos_obj_create(bss->attr_sos, NULL, bss->pattern_attribute_schema, obj);
 out:
	if (obj)
		sos_obj_put(obj);
	dsos_transaction_end(bss->attr_sos);
		...
		const char *attr_type, const char *attr_value)
{
	...
	obj = dsos_index_find(bss->attr_sos, bss->pattern_attribute_schema,
			bss->pa_ptv_attr, key);
	if (obj) {
		rc = EEXIST;
		goto out;
		...
		const char *attr_type, const char *attr_value)
{
	...
	dsos_obj_delete(bss->attr_sos, obj);
 out:
	if (obj)
		sos_obj_put(obj);
	dsos_transaction_end(bss->attr_sos);
			     ...
			     const char *attr_type)
{
	...
	sos_key_join(key, bss->pa_ptv_attr, ptn_id, t_len, attr_type, 0, "");

 next_obj:
	dsos_transaction_begin(bss->attr_sos, NULL);
	obj = dsos_index_find_ge(bss->attr_sos, bss->pattern_attribute_schema,
				 bss->pa_ptv_attr, key);
	if (!obj) {
		if (errno == ENOENT) {
			/* OK */
	...
	val = sos_obj_ptr(obj);
	v = sos_value(obj, bss->pa_at_attr);
	if (val->ptn_id != ptn_id ||
			0 != strcmp(attr_type, v->data->array.data.char_)) {
		/* depleted */
		sos_value_put(v);
		rc = 0;
		goto out;
...
static char *bs_ptn_attr_get(bstore_t bs, bptn_id_t ptn_id, const char *attr_type)
{
	...
	rc = bs_attr_find(bs, attr_type);
	if (rc)  {
		/* no such attr_type, do not proceed */
		errno = rc;
		return NULL;
	}
	...
	v = sos_value(obj, bss->pa_av_attr);
	if (!v) {
		rc = errno;
		goto out;
	}
	ret = malloc(v->data->array.count);
	if (!ret) {
		rc = ENOMEM;
		sos_value_put(v);
		goto out;
	}
	memcpy(ret, v->data->array.data.char_, v->data->array.count);
	sos_value_put(v);
 out:
	if (obj)
		sos_obj_put(obj);
...
static bptn_attr_iter_t bs_ptn_attr_iter_new(bstore_t bs)
{
	...
	if (!mi)
		return NULL;
	mi->bs = bs;
	mi->biter_type = BPTN_ATTR_ITER;
	mi->iter_type = PTN_ATTR_ITER_PTV;
	mi->iter = dsos_iter_create(bss->attr_sos, bss->pattern_attribute_schema,
				    "ptn_type_value");
	if (!mi->iter) {
		free(mi);
		return NULL;
...
static void bs_ptn_attr_iter_free(bptn_attr_iter_t i)
{
	bsos_iter_t _i = (bsos_iter_t)i;
	if (_i->obj)
		sos_obj_put(_i->obj);
	if (_i->iter)
		dsos_iter_delete(_i->iter);
	free(i);
}
				       ...
				       bstore_iter_filter_t filter)
{
	...
	bstore_dsos_t bss = (bstore_dsos_t)pai->bs;
	int type;
	int rc;
	...
	if (pai->iter_type != type) {
		pai->iter_type = type;
		if (pai->iter) {
			dsos_iter_delete(pai->iter);
			pai->iter = NULL;
		}
		switch (type) {
		case PTN_ATTR_ITER_PTV:
			pai->iter = dsos_iter_create(bss->attr_sos,
					bss->pattern_attribute_schema,
					"ptn_type_value");
			break;
				     ...
				     const char *attr_value)
{
	...
	if (pai->obj)
		sos_obj_put(pai->obj);
	pai->obj = dsos_iter_find_ge(pai->iter, key);
	if (!pai->obj) {
		rc = errno;
		goto err_0;
	}
...
static int bs_ptn_attr_iter_next(bptn_attr_iter_t iter)
{
	...
	if (pai->obj)
		sos_obj_put(pai->obj);
	pai->obj = dsos_iter_next(pai->iter);
	if (!pai->obj) {
		if (errno)
			return errno;
		return ENOENT;
...
{
	snprintf(ver->ver, sizeof(ver->ver), "1.0");
	snprintf(ver->gitsha, sizeof(ver->gitsha), "%s", GITSHA);
	return 0;
}
